...
export function launchBot(token){
    ...
    bot.launch().then(async () => {
        ...
        //await bot.telegram.replyWithPhoto({ url: 'https://pbs.twimg.com/profile_banners/1463071986515599362/1709730824/1500x500' }, {
        //    caption: "Welcome to Dugger King Bot! Here is some important information:"
        // });
...
function listenToCommands(bot) {
    ...
    bot.start(async (ctx) => {
        await ctx.reply("Welcome to wolf bot. Click on the button to launch our mini app", {
            reply_markup: {
                inline_keyboard: [
                    [ { text: "Click on the button to launch", web_app: { url: process.env.APP_URL } } ],
                ]
            }
        });
